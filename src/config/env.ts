...
export function parseEnv(customEnv?: Record<string, string | undefined>) {
  ...
  const backendRaw = getEnv("VITE_DATA_BACKEND")?.trim() || "preview";
  ...
  const branchModeRaw = getEnv("VITE_BRANCH_MODE")?.trim() || "single";
  if (branchModeRaw !== "single" && branchModeRaw !== "multi") {
    throw new EnvironmentConfigurationError("UNSUPPORTED_BRANCH_CONFIGURATION");
  }
  ...
  if (backend === "preview") {
    ...
    if (!rawCenterId || !validateUUID(rawCenterId)) {
      if (!customEnv && import.meta.env.MODE === "test") {
        ...
        throw new EnvironmentConfigurationError("MISSING_SINGLE_BRANCH_CENTER_ID");
      }
    }
