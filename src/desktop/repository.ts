...
import { exportDesktopBackup, getDesktopDatabaseHealth, importDesktopBackup, printDesktopHtml } from './sqlite';
...
export const desktopRepository = {
  ...
  printHtml: async (title: string, html: string) => {
    try {
      return { ok: true as const, data: await printDesktopHtml({ title, html }) };
    } catch {
