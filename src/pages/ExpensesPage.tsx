...
export default function ExpensesPage() {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setNewDesc(""); setNewAmount(""); setNewCategory("Supplies");
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      closeEdit();
      showToast("success", t("Success"), t("Expense updated successfully"));
      load();
    } catch (e) {
      showToast("error", t("Error"), (e as Error).message || String(e));
    }
                    ...
                    <td>
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="h-12 w-12 rounded-2xl border border-border bg-card flex items-center justify-center text-muted-foreground hover:bg-amber-500/10 hover:text-amber-600 hover:border-amber-500/20 transition-all shadow-sm hover:scale-110 active:scale-95"
                          title={t("Edit")}
                        >
            ...
            {filtered.map((exp, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    onClick={() => openEdit(exp)}
                    className="h-12 flex-1 rounded-2xl border border-border bg-card flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-wider text-amber-600 hover:bg-amber-500/10 hover:border-amber-500/20 transition-all shadow-sm"
                  >
              ...
              <div className="p-6 sm:p-10 space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <input className="w-full rounded-[1.5rem] border border-border bg-muted/30 ps-14 pe-6 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all" value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder={t("e.g., Electricity Bill")} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input type="number" className="w-full rounded-[1.5rem] border border-border bg-muted/30 ps-14 pe-6 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all" value={newAmount} onChange={(e) => setNewAmount(e.target.value)} />
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <select className="w-full rounded-[1.5rem] border border-border bg-muted/30 ps-14 pe-10 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all appearance-none" value={newCategory} onChange={(e) => setNewCategory(e.target.value)}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 40 }} className="relative w-full max-w-lg rounded-[1.5rem] sm:rounded-[3rem] border border-border bg-card shadow-2xl overflow-hidden">
              ...
              <div className="p-6 sm:p-10 space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <Receipt className="absolute start-5 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                    <input className="w-full rounded-[1.5rem] border border-border bg-muted/30 ps-14 pe-6 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all" value={editDesc} onChange={(e) => setEditDesc(e.target.value)} placeholder={t("e.g., Electricity Bill")} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <DollarSign className="absolute start-5 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <input type="number" className="w-full rounded-[1.5rem] border border-border bg-muted/30 ps-14 pe-6 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all" value={editAmount} onChange={(e) => setEditAmount(e.target.value)} />
                    </div>
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] ms-2">{t("Category")}</label>
                    <div className="relative">
                      <Tag className="absolute start-5 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <select className="w-full rounded-[1.5rem] border border-border bg-muted/30 ps-14 pe-10 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all appearance-none" value={editCategory} onChange={(e) => setEditCategory(e.target.value)}>
                        {categories.map(c => <option key={c} value={c}>{t(c)}</option>)}
                      </select>
                    </div>
                  </div>
