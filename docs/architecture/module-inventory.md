...
- **LoginPage**: System entry point handling Preview and Auth operations.
...
- `route-guards.tsx`: Implements role detection (`RequireAuth`, `RequireAdmin`), enforcing `preview` vs `authenticated` routing state.
- `AppRoutes`: Provides centralized hierarchy tracking layout scopes.

### Infrastructure Domains
- `infrastructure/preview`: Contains Safe read-only mock structures isolating demonstration data away from component logic.
